        ...
        <div className="relative aspect-[1/1.18]" style={{ transform: "rotateY(-12deg) rotateX(5deg)", transformStyle: "preserve-3d" }}>
          ...
          {[0, 1, 2].map((floor) => (
            <motion.div
              key={floor}
              style={{ y: lift[floor] }}
              className="absolute right-[11%] left-[8%] h-[23%] border border-sand-dim/70 bg-sand shadow-lift"
              data-floor={floor + 1}
            >
            ...
            <div key={floor} className="absolute inset-x-0" style={{ bottom: `${12 + floor * 24}%`, height: "23%" }}>{element}</div>
          ))}
